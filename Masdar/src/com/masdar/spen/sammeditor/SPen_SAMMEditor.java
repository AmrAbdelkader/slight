package com.masdar.spen.sammeditor;
...
import com.masdar.spen.SPenSDK_Demo_Editor;
import com.masdar.spen.basiceditor.SPen_BasicEditor;
import com.masdar.spen.tools.PreferencesOfAnimationOption;
import com.masdar.spen.tools.PreferencesOfOtherOption;
import com.masdar.spen.tools.PreferencesOfSAMMOption;
import com.masdar.spen.tools.SPenSDKUtils;
import com.masdar.spen.tools.ToolAudioListView;
import com.masdar.spen.tools.ToolColorPickerDialog;
import com.masdar.spen.tools.ToolFileTotalInfoShow;
import com.masdar.spen.tools.ToolListActivity;
import com.masdar.spen.tools.ToolTextDialogInput;
import com.masdar.spen.tools.ToolColorPickerDialog.OnColorChangedListener;
...
public class SPen_SAMMEditor extends Activity {
	...
	private final int SET_BACKGROUND = 1;
	private final int CLEAR_BACKGROUND = 2;
		...
		mSCanvas.setFileProcessListener(new FileProcessListener() {
			...
			public void onLoadComplete(boolean bLoadResult) {			 
				if(bLoadResult){
					...
					Toast.makeText(SPen_SAMMEditor.this, "Load AMS File("+ appID + ") Success!", Toast.LENGTH_LONG).show();
				}
				else{
					Toast.makeText(SPen_SAMMEditor.this, "Load AMS File Fail!", Toast.LENGTH_LONG).show();				
				}
			}
		});
	...
	private OnClickListener mInsertBtnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if (v.equals(mInsertBtn)) {	
				String items[] = {"Insert Image", "Set Background" , "Clear Background"};
	...
	private void insertVideoObjectSelection(){
		...
		.setItems(items, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				switch (which) {
				case 0:	// Video file
					Intent intent = new Intent(SPen_SAMMEditor.this, SPen_Example_VideoDemoFileList.class);
					startActivityForResult(intent, REQUEST_CODE_INSERT_VIDEO_OBJECT);					
					break;
				case 1: 	// Video URL Link
					LayoutInflater factory = LayoutInflater.from(SPen_SAMMEditor.this);
					...
					AlertDialog dlg = new AlertDialog.Builder(SPen_SAMMEditor.this)
					.setTitle("Video URL")
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode==REQUEST_CODE_FILE_SELECT){
			...
			mbPreviewBtnClick = data.getBooleanExtra(SPen_AnimationViewer.EXTRA_PLAY_BUTTON_CLICK, false);
		}
	...
	//Amr_Abdelkader uploading animation
	void uploadAnimation(){
		String tempSAMMFileName = mTempAMSFolderPath + "/" + "tempsammfile.png";
		saveTempSAMM(tempSAMMFileName);
		final FileUploader fileUploader = new FileUploader(mContext, tempSAMMFileName, "image_file" , 2);
		fileUploader.uploadIdea(3, userId);
	}
	...
	void previewAnimation(){
		...
		Intent intent = new Intent(this, SPen_AnimationViewer.class);
		intent.putExtra(SPen_AnimationViewer.EXTRA_VIEW_FILE_PATH, sDataKey);
		intent.putExtra(SPen_AnimationViewer.EXTRA_PLAY_CANVAS_WIDTH, mCanvasWidth);
		intent.putExtra(SPen_AnimationViewer.EXTRA_PLAY_CANVAS_HEIGHT, mCanvasHeight);
		intent.putExtra(SPen_AnimationViewer.EXTRA_CONTENTS_ORIENTATION, mbContentsOrientationHorizontal);
		intent.putExtra(SPen_AnimationViewer.EXTRA_SINGLE_SELECTION_LAYER_MODE, mbSingleSelectionFixedLayerMode);
	...
	//Amr_Abdelkader - Editing set Image & Background
...
//		moreMenu.add(MENU_MORE_GROUP, MENU_MORE_OTHER_OPTION, 5, "Other Option");
	...
	private void checkSameSaveFileName(final String saveFileName) {	
		...
		if(fSaveFile.exists())
		{
			...
			saveNewSAMMTempFile(saveFileName);			
		}
	}

	public void saveNewSAMMTempFile(final String saveFileName) {
		// canvas option setting
		SOptionSCanvas canvasOption = new SOptionSCanvas();
		// Cropping option 
		canvasOption.mSAMMOption.setSaveImageLeftCroppingOption(PreferencesOfSAMMOption.getPreferenceSaveImageHorizontalCrop(mContext));
	...
	public void saveTempSAMM(final String saveFileName) {
		File fSaveFile = new File(saveFileName);
		if(fSaveFile.exists())
		{
			// canvas option setting
		...
		.setItems(R.array.backgroundimageoperation, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {			
				if(!mSCanvas.filterBGImage(imageOperationByIndex[which], imageOperationLevel))
					Toast.makeText(SPen_SAMMEditor.this, "To apply backGround image effect, set background image firstly or select valid image effect!!!", Toast.LENGTH_LONG).show();
			}
		})
		.show();
	}
		...
		.setPositiveButton("OK", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				if(!mSCanvas.setCheckPreference(mPreferenceCheckedItem))
					Toast.makeText(SPen_SAMMEditor.this, "Fail to set preference normal!", Toast.LENGTH_LONG).show();
				else
					if(mPreferenceCheckedItem == 0)
						Toast.makeText(SPen_SAMMEditor.this, "Current Preference = Normal", Toast.LENGTH_LONG).show();
					else if(mPreferenceCheckedItem == 1)
						Toast.makeText(SPen_SAMMEditor.this, "Current Preference = Favorite", Toast.LENGTH_LONG).show();
					else if(mPreferenceCheckedItem == 2)
						Toast.makeText(SPen_SAMMEditor.this, "Current Preference = Custom", Toast.LENGTH_LONG).show();
			}
		})
		.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
