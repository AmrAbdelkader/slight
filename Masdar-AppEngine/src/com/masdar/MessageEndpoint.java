 ...
 */
@Api(name = "messageEndpoint", namespace = @ApiNamespace(ownerDomain = "masdar.com", ownerName = "masdar.com", packagePath=""))
...
public class MessageEndpoint {
  ...
  private static final String API_KEY = "AIzaSyCmw2naK3lzObCNDWNl2-21RnfrCZie0DQ";
  ...
  @ApiMethod(name = "sendMessageToDevice")
  public void sendMessageToDevice(@Named("message") String message , @Named("deviceId") String deviceId)
      throws IOException {
    Sender sender = new Sender(API_KEY);
    // create a MessageData entity with a timestamp of when it was
    // received, and persist it
    ...
    EntityManager mgr = getEntityManager();
    try {
    	Query query = mgr.createQuery("select from DeviceInfo d WHERE d.deviceInformation = '"+deviceId+"'");
		@SuppressWarnings("unchecked")
		List<DeviceInfo> devicesFound = (List<DeviceInfo>) query.getResultList(); 
    	if(devicesFound.size() != 0){
    		deviceInfo = devicesFound.get(0);
    		mgr.persist(messageObj);    	
        	doSendViaGcm(message, sender, deviceInfo);
    	}
    } finally {
